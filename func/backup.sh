...
local_backup() {
	...
	if [ "$disk_usage" -ge "$BACKUP_DISK_LIMIT" ]; then
		...
		sed -i "/ $user /d" $SKYNET/data/queue/backup.pipe
	...
	chown "skynetweb":"$user" $BACKUP/$user.$backup_new_date.tar
	localbackup='yes'
...
ftp_backup() {
	# Checking config
	if [ ! -e "$SKYNET/conf/ftp.backup.conf" ]; then
	...
	# Parse config
	source_conf "$SKYNET/conf/ftp.backup.conf"

	# Set default port
	if [ -z "$(grep 'PORT=' $SKYNET/conf/ftp.backup.conf)" ]; then
		PORT='21'
	fi
...
sftp_download() {
	source_conf "$SKYNET/conf/sftp.backup.conf"
	if [ -z "$PORT" ]; then
		PORT='22'
	fi
	cd $BACKUP
...
sftp_backup() {
	# Checking config
	if [ ! -e "$SKYNET/conf/sftp.backup.conf" ]; then
	...
	# Set default port
	if [ -z "$(grep 'PORT=' $SKYNET/conf/sftp.backup.conf)" ]; then
		PORT='22'
	fi
...
b2_backup() {
	...
	source_conf "$SKYNET/conf/b2.backup.conf"
...
rclone_backup() {
	...
	source_conf "$SKYNET/conf/rclone.backup.conf"

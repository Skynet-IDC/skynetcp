...
#===========================================================================#
#                                                                           #
# skynet Control Panel - Core Function Library                              #
#                                                                           #
#===========================================================================#
...
if [ -z "$user" ]; then
	if [ -z "$ROOT_USER" ]; then
		...
		source_conf "$SKYNET/conf/skynet.conf" # load config file
	fi
	user="$ROOT_USER"
fi
...
skynet_GIT_REPO="https://raw.githubusercontent.com/skynetcp/skynetcp"
...
log_event() {
	...
	if [ "$1" -eq 0 ]; then
		echo "$LOG_TIME $2" >> $SKYNET/log/system.log
	else
		echo "$LOG_TIME $2 [Error $1]" >> $SKYNET/log/error.log
	fi
}
...
log_history() {
	...
	if [ "$log_user" = "system" ]; then
		log=$SKYNET/log/activity.log
	else
		if ! $BIN/v-list-user "$log_user" > /dev/null; then
			return $E_NOTEXIST
		fi
		log=$SKYNET/data/users/$log_user/history.log
	fi
	touch $log
...
is_package_valid() {
	if [ -z $1 ]; then
		if [ ! -e "$SKYNET/data/packages/$package.pkg" ]; then
			check_result "$E_NOTEXIST" "package $package doesn't exist"
		fi
	else
		if [ ! -e "$SKYNET/data/packages/$1.pkg" ]; then
...
# Check user backup settings
is_backup_scheduled() {
	if [ -e "$SKYNET/data/queue/backup.pipe" ]; then
		check_q=$(grep " $user " $SKYNET/data/queue/backup.pipe | grep $1)
		if [ -n "$check_q" ]; then
...
is_object_valid() {
	if [ $2 = 'USER' ]; then
		tstpath="$(readlink -f "$SKYNET/data/users/$3")"
		if [ "$(dirname "$tstpath")" != "$(readlink -f "$SKYNET/data/users")" ] || [ ! -d "$SKYNET/data/users/$3" ]; then
			check_result "$E_NOTEXIST" "$1 $3 doesn't exist"
		fi
	elif [ $2 = 'KEY' ]; then
		local key="$(basename "$3")"

		if [[ -z "$key" || ${#key} -lt 16 ]] || [[ ! -f "$SKYNET/data/access-keys/${key}" && ! -f "$SKYNET/data/access-keys/$key" ]]; then
			check_result "$E_NOTEXIST" "$1 $3 doesn't exist"
		fi
	else
		object=$(grep "$2='$3'" $SKYNET/data/users/$user/$1.conf)
		if [ -z "$object" ]; then
			arg1=$(basename $1)
...
update_user_value() {
	key="${2//$/}"
	lnr=$(grep -n "^$key='" $SKYNET/data/users/$1/user.conf | cut -f 1 -d ':')
	if [ -n "$lnr" ]; then
		sed -i "$lnr d" $SKYNET/data/users/$1/user.conf
		sed -i "$lnr i\\$key='${3}'" $SKYNET/data/users/$1/user.conf
	fi
}
...
increase_user_value() {
	key="${2//$/}"
	factor="${3-1}"
	conf="$SKYNET/data/users/$1/user.conf"
...
is_ip_format_valid() {
	object_name=${2-ip}
	valid=$($SKYNET_PHP -r '$ip="$argv[1]"; echo (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
...
is_ipv6_format_valid() {
	object_name=${2-ipv6}
	valid=$($SKYNET_PHP -r '$ip="$argv[1]"; echo (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
		check_result "$E_INVALID" "invalid $object_name :: $1"
	fi
}
...
is_ip46_format_valid() {
	valid=$($SKYNET_PHP -r '$ip="$argv[1]"; echo (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4 | FILTER_FLAG_IPV6) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
...
is_ipv4_cidr_format_valid() {
	object_name=${2-ip}
	valid=$($SKYNET_PHP -r '$cidr="$argv[1]"; list($ip, $netmask) = [...explode("/", $cidr), 32]; echo ((filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) && $netmask <= 32) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
		check_result "$E_INVALID" "invalid $object_name :: $1"
	fi
}
...
is_ipv6_cidr_format_valid() {
	object_name=${2-ipv6}
	valid=$($SKYNET_PHP -r '$cidr="$argv[1]"; list($ip, $netmask) = [...explode("/", $cidr), 128]; echo ((filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) && $netmask <= 128) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
		check_result "$E_INVALID" "invalid $object_name :: $1"
	fi
}
...
is_netmask_format_valid() {
	...
	valid=$($SKYNET_PHP -r '$netmask="$argv[1]"; echo (preg_match("/^(128|192|224|240|248|252|254|255)\.(0|128|192|224|240|248|252|254|255)\.(0|128|192|224|240|248|252|254|255)\.(0|128|192|224|240|248|252|254|255)/", $netmask) ? 0 : 1);' $1)
	if [ "$valid" -ne 0 ]; then
		check_result "$E_INVALID" "invalid $object_name :: $1"
	fi
}
...
check_access_key_secret() {
	...
	if [[ -z "$access_key_id" || ! -f "$SKYNET/data/access-keys/${access_key_id}" ]]; then
	...
	if [[ -z "$secret_access_key" ]]; then
		...
		SECRET_ACCESS_KEY=""
		source_conf "$SKYNET/data/access-keys/${access_key_id}"
...
check_access_key_cmd() {
	...
	if [[ "$DEBUG_MODE" = "true" ]]; then
		new_timestamp
		echo "[$date:$time] $1 $2" >> /var/log/skynet/api.log
	fi
	if [[ -z "$access_key_id" || ! -f "$SKYNET/data/access-keys/${access_key_id}" ]]; then
		check_result "$E_FORBIDEN" "Access key $access_key_id doesn't exist"
	fi
...
check_skynet_demo_mode() {
	demo_mode=$(grep DEMO_MODE /usr/local/skynet/conf/skynet.conf | cut -d '=' -f2 | sed "s|'||g")
...
is_skynet_package() {
	...
	if [ "$check" != "true" ]; then
		check_result $E_INVALID "$2 package is not controlled by skynetcp"
	fi
}
...
change_sys_value() {
	check_ckey=$(grep "^$1='" "$SKYNET/conf/skynet.conf")
	if [ -z "$check_ckey" ]; then
		echo "$1='$2'" >> "$SKYNET/conf/skynet.conf"
	else
		sed -i "s|^$1=.*|$1='$2'|g" "$SKYNET/conf/skynet.conf"
	fi
}
...
is_key_permissions_format_valid() {
	...
	while IFS=',' read -ra permissions_arr; do
		for permission in "${permissions_arr[@]}"; do
			...
			if [[ ! -e "$SKYNET/data/api/$permission" ]]; then
				check_result "$E_NOTEXIST" "API $permission doesn't exist"
			fi

			source_conf "$SKYNET/data/api/$permission"
...
# Get the position of an argument by name in a skynet command using the command's documentation comment.
#
# Return:

...
add_upgrade_message() {
	if [ -f "$SKYNET_BACKUP/message.log" ]; then
		echo -e $1 >> $SKYNET_BACKUP/message.log
		echo -e "\n\n" >> $SKYNET_BACKUP/message.log
	else
		echo -e $1 > $SKYNET_BACKUP/message.log
	fi
}
...
upgrade_health_check() {
	...
	# Perform basic health check against skynet.conf to ensure that
...
upgrade_welcome_message() {
	...
	echo ""
	echo "Backup directory: $SKYNET_BACKUP/"
...
upgrade_send_notification_to_email() {
	...
	if [ "$UPGRADE_SEND_EMAIL" = "true" ]; then
		...
		message_tmp_file="/tmp/skynet-upgrade-complete.txt"
		...
		if [[ -f "$SKYNET_BACKUP/message.log" ]]; then
			...
			echo "" >> $message_tmp_file
			cat $SKYNET_BACKUP/message.log >> $message_tmp_file
			echo "" >> $message_tmp_file
			echo "===================================================" >> $message_tmp_file
			echo "" >> $message_tmp_file
		fi
...
upgrade_config_set_value() {
	if [ -f "$SKYNET_BACKUP/upgrade.conf" ]; then
		if [ "$2" = "true" ]; then
			sed -i "s/$1='false'/$1='true'/g" $SKYNET_BACKUP/upgrade.conf
		fi
	fi
}

prepare_upgrade_config() {
	mkdir -p $SKYNET_BACKUP
	touch $SKYNET_BACKUP/upgrade.conf
	while IFS='= ' read -r lhs rhs; do
		if [[ ! $lhs =~ ^\ *# && -n $lhs ]]; then
			...
			echo "$lhs='$rhs'" >> $SKYNET_BACKUP/upgrade.conf
		fi
	done < "$SKYNET/install/upgrade/upgrade.conf"
}

upgrade_init_backup() {
	# Ensure that backup directories are created
	# skynet Control Panel configuration files
	mkdir -p $SKYNET_BACKUP/conf/skynet/

	# OpenSSL configuration files
	mkdir -p $SKYNET_BACKUP/conf/openssl/

	# Hosting Packages
	mkdir -p $SKYNET_BACKUP/packages/

	# Domain template files
	mkdir -p $SKYNET_BACKUP/templates/

	# System services (apache2, nginx, bind9, vsftpd, etc).
	if [ -n "$WEB_SYSTEM" ]; then
		mkdir -p $SKYNET_BACKUP/conf/$WEB_SYSTEM/
	fi
	if [ -n "$IMAP_SYSTEM" ]; then
		mkdir -p $SKYNET_BACKUP/conf/$IMAP_SYSTEM/
	fi
	if [ -n "$MAIL_SYSTEM" ]; then
		mkdir -p $SKYNET_BACKUP/conf/$MAIL_SYSTEM/
	fi
	if [ -n "$DNS_SYSTEM" ]; then
		mkdir -p $SKYNET_BACKUP/conf/$DNS_SYSTEM/
	fi
	if [ -n "$PROXY_SYSTEM" ]; then
		mkdir -p $SKYNET_BACKUP/conf/$PROXY_SYSTEM/
	fi
	if [ -n "$DB_SYSTEM" ]; then
		if [[ "$DB_SYSTEM" =~ "mysql" ]]; then
			mkdir -p $SKYNET_BACKUP/conf/mysql/
		fi
		if [[ "$DB_SYSTEM" =~ "pgsql" ]]; then
			mkdir -p $SKYNET_BACKUP/conf/pgsql/
		fi
	fi
	if [ -n "$FTP_SYSTEM" ]; then
		mkdir -p $SKYNET_BACKUP/conf/$FTP_SYSTEM/
	fi
	if [ -n "$FIREWALL_SYSTEM" ]; then
		mkdir -p $SKYNET_BACKUP/conf/$FIREWALL_SYSTEM/
	fi
	if [ -n "$FIREWALL_EXTENSION" ]; then
		mkdir -p $SKYNET_BACKUP/conf/$FIREWALL_EXTENSION/
	fi
	if [ -e "/etc/ssh/sshd_config" ]; then
		mkdir -p $SKYNET_BACKUP/conf/ssh/
	fi
	if [ -d "/etc/roundcube/" ]; then
		mkdir -p $SKYNET_BACKUP/conf/roundcube/
	fi
	if [ -d "/etc/snappymail/" ]; then
		mkdir -p $SKYNET_BACKUP/conf/snappymail/
	fi
	if [ -d "/etc/phpmyadmin/" ]; then
		mkdir -p $SKYNET_BACKUP/conf/phpmyadmin/
	fi
	if [ -d "/etc/phppgadmin/" ]; then
		mkdir -p $SKYNET_BACKUP/conf/phppgadmin/
	fi
}

upgrade_init_logging() {
	# Set log file path
	LOG="$SKYNET_BACKUP/hst-upgrade-$(date +%d%m%Y%H%M).log"
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Development build for testing purposes only. Report bugs at https://github.com/skynetcp/skynetcp/issues/."
	fi
	if [[ "$new_version" =~ "beta" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Beta release. Please report bugs at https://github.com/skynetcp/skynetcp/issues/."
	fi
}
...
upgrade_start_backup() {
	...
	cp -fr $SKYNET/data/packages/* $SKYNET_BACKUP/packages/

	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - Templates"
	fi
	cp -fr $SKYNET/data/templates/* $SKYNET_BACKUP/templates/
	...
	cp -fr $SKYNET/conf/* $SKYNET_BACKUP/conf/skynet/

	# OpenSSL configuration files
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      ---- openssl"
	fi
	cp -f /etc/ssl/*.cnf $SKYNET_BACKUP/conf/openssl/
	...
	if [ -n "$WEB_SYSTEM" ]; then
		...
		cp -fr /etc/$WEB_SYSTEM/* $SKYNET_BACKUP/conf/$WEB_SYSTEM/
	fi
	if [ -n "$PROXY_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $PROXY_SYSTEM"
		fi
		cp -fr /etc/$PROXY_SYSTEM/* $SKYNET_BACKUP/conf/$PROXY_SYSTEM/
	fi
	if [ -n "$IMAP_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $IMAP_SYSTEM"
		fi
		cp -fr /etc/$IMAP_SYSTEM/* $SKYNET_BACKUP/conf/$IMAP_SYSTEM/
	fi
	if [ -n "$MAIL_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $MAIL_SYSTEM"
		fi
		cp -fr /etc/$MAIL_SYSTEM/* $SKYNET_BACKUP/conf/$MAIL_SYSTEM/
	fi
	if [ -n "$DNS_SYSTEM" ]; then
		if [ "$DNS_SYSTEM" = "bind9" ]; then
			...
			cp -fr /etc/bind/* $SKYNET_BACKUP/conf/$DNS_SYSTEM/
		fi
	fi
	if [ -n "$DB_SYSTEM" ]; then
		if [[ "$DB_SYSTEM" =~ "mysql" ]]; then
			...
			cp -fr /etc/mysql/* $SKYNET_BACKUP/conf/mysql/
		fi
		if [[ "$DB_SYSTEM" =~ "pgsql" ]]; then
			if [ "$DEBUG_MODE" = "true" ]; then
				echo "      ---- pgsql"
			...
			cp -fr /etc/postgresql/* $SKYNET_BACKUP/conf/pgsql/
		fi
	fi
	if [ -n "$FTP_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
		...
		if [ "$FTP_SYSTEM" = "vsftpd" ]; then
			cp -f /etc/$FTP_SYSTEM.conf $SKYNET_BACKUP/conf/$FTP_SYSTEM/
		fi
		if [ "$FTP_SYSTEM" = "proftpd" ]; then
			cp -f /etc/proftpd/proftpd.conf $SKYNET_BACKUP/conf/$FTP_SYSTEM/
		fi
	fi
	if [ -n "$FIREWALL_SYSTEM" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
		...
		[ -e "/etc/sysconfig/iptables" ] && cp -f /etc/sysconfig/iptables $SKYNET_BACKUP/conf/$FIREWALL_SYSTEM/
		[ -e "/etc/iptables.rules" ] && cp -f /etc/iptables.rules $SKYNET_BACKUP/conf/$FIREWALL_SYSTEM/
	fi
	if [ -n "$FIREWALL_EXTENSION" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- $FIREWALL_EXTENSION"
		fi
		cp -f /etc/$FIREWALL_EXTENSION/*.conf $SKYNET_BACKUP/conf/$FIREWALL_EXTENSION/
		cp -f /etc/$FIREWALL_EXTENSION/*.local $SKYNET_BACKUP/conf/$FIREWALL_EXTENSION/
	fi
	if [ -e "/etc/ssh/sshd_config" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- sshd"
		fi
		cp -fr /etc/ssh/* $SKYNET_BACKUP/conf/ssh/
	fi
	if [ -d "/etc/roundcube" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- Roundcube"
		fi
		cp -fr /etc/roundcube/* $SKYNET_BACKUP/conf/roundcube
	fi
	if [ -d "/etc/snappymail" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- SnappyMail"
		fi
		cp -fr /etc/snappymail/* $SKYNET_BACKUP/conf/snappymail
	fi
	if [ -d "/etc/phpmyadmin" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- phpMyAdmin"
		fi
		cp -fr /etc/phpmyadmin/* $SKYNET_BACKUP/conf/phpmyadmin
	fi
	if [ -d "/etc/phppgadmin" ]; then
		if [ "$DEBUG_MODE" = "true" ]; then
			echo "      ---- phppgadmin"
		fi
		cp -fr /etc/phppgadmin/* $SKYNET_BACKUP/conf/phppgadmin
	fi
}

upgrade_refresh_config() {
	source_conf "/usr/local/skynet/conf/skynet.conf"
}
...
upgrade_start_routine() {
	...
	upgrade_steps=$(ls -v $SKYNET/install/upgrade/versions/*.sh)
	for script in $upgrade_steps; do
		declare -a available_versions
		available_versions+=($(echo $script | sed "s|/usr/local/skynet/install/upgrade/versions/||g" | sed "s|.sh||g"))
	done
	...
	if [ $(check_version $latest_version) -gt $(check_version $VERSION) ]; then
		for version_step in "${available_versions[@]}"; do
			if [ $(check_version $VERSION) -lt $(check_version "$version_step") ]; then
				upgrade_step_message
				source $SKYNET/install/upgrade/versions/$version_step.sh
			fi
		done
		...
		echo ""
		echo "[ ! ] The latest version of skynet Control Panel is already installed."
		echo "      Verifying configuration..."
		echo ""
		if [ -e "$SKYNET/install/upgrade/versions/$VERSION.sh" ]; then
			source $SKYNET/install/upgrade/versions/$VERSION.sh
		fi
		VERSION="$new_version"
		upgrade_set_version "$VERSION"
		upgrade_refresh_config
	...
	if [ -n "$(echo $DB_SYSTEM | grep -w 'mysql')" ]; then
		...
		if version_ge "$pma_version" "$pma_v"; then
			...
			if [ -e /var/lib/phpmyadmin/blowfish_secret.inc.php ]; then
				chown root:skynetmail /var/lib/phpmyadmin/blowfish_secret.inc.php
				chmod 0640 /var/lib/phpmyadmin/blowfish_secret.inc.php
			fi
			chown skynetmail:skynetmail /usr/share/phpmyadmin/tmp
...
upgrade_filemanager() {
	FILE_MANAGER_CHECK=$(cat $SKYNET/conf/skynet.conf | grep "FILE_MANAGER='false'")
	if [ -z "$FILE_MANAGER_CHECK" ]; then
		if [ -f "$SKYNET/web/fm/version" ]; then
			fm_version=$(cat $SKYNET/web/fm/version)
		else
			fm_version="1.0.0"
		fi
		if ! version_ge "$fm_version" "$fm_v"; then
			...
			if [ "$UPGRADE_UPDATE_FILEMANAGER_CONFIG" = "true" ]; then
				if [ -e "$SKYNET/web/fm/configuration.php" ]; then
					echo "[ ! ] Updating File Manager configuration..."
					# Update configuration.php
					cp -f $SKYNET_INSTALL_DIR/filemanager/filegator/configuration.php $SKYNET/web/fm/configuration.php

					# Path to the file manager configuration file where the change will be made.
					config_file="$SKYNET/web/fm/configuration.php"
...
upgrade_roundcube() {
	if [ -n "$(echo "$WEBMAIL_SYSTEM" | grep -w 'roundcube')" ]; then
		if [ -d "/usr/share/roundcube" ]; then
			...
			echo "      To upgrade to the latest version of Roundcube directly from upstream, from please run the command migrate_roundcube.sh located in: /usr/local/skynet/install/upgrade/manual/"
		else
...
upgrade_dependencies() {
	echo "[ ! ] Update skynet PHP dependencies..."
...
upgrade_restart_services() {
	if [ "$UPGRADE_RESTART_SERVICES" = "true" ]; then
		...
		if [ "$WEB_TERMINAL" = "true" ]; then
			...
			$BIN/v-restart-service "skynet-web-terminal"
		fi
	...
	# Always restart the skynet Control Panel service
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - skynet"
	fi
	$BIN/v-restart-service skynet
}

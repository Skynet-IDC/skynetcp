...
is_web_domain_new() {
	web=$(grep -F -H "DOMAIN='$1'" $SKYNET/data/users/*/web.conf)
	if [ -n "$web" ]; then
		if [ "$type" == 'web' ]; then
...
is_web_alias_new() {
	grep -wH "$1" $SKYNET/data/users/*/web.conf | while read -r line; do
...
prepare_web_domain_values() {
	...
	if [ "$SUSPENDED" = 'yes' ]; then
		docroot="$SKYNET/data/templates/web/suspend"
		sdocroot="$SKYNET/data/templates/web/suspend"
...
is_dns_domain_new() {
	dns=$(ls $SKYNET/data/users/*/dns/$1.conf 2> /dev/null)
	if [ -n "$dns" ]; then
		if [ "$2" == 'dns' ]; then
...
is_mail_domain_new() {
	mail=$(ls $SKYNET/data/users/*/mail/$1.conf 2> /dev/null)
	if [ -n "$mail" ]; then
		if [ "$2" == 'mail' ]; then
	...
	for mail_reserved in $(echo "mail $WEBMAIL_ALIAS"); do
		if [ -n "$(ls $SKYNET/data/users/*/mail/$mail_reserved.$1.conf 2> /dev/null)" ]; then
			if [ "$2" == 'mail' ]; then
				check_result "$E_EXISTS" "Required subdomain \"$mail_reserved.$1\" already exists"
			fi
		fi
		if [ -n "$(ls $SKYNET/data/users/*/mail/$mail_nosub.conf 2> /dev/null)" ] && [ "$mail_sub" = "$mail_reserved" ]; then
			if [ "$2" == 'mail' ]; then
...
add_mail_ssl_config() {
	...
	if [ ! -d "$SKYNET/ssl/mail" ]; then
		mkdir -p $SKYNET/ssl/mail
	fi
	...
	# Add certificate to skynet user configuration data directory
	...
	if [ -n "$mail_cert_match" ]; then
		...
		ln -s $HOMEDIR/$user/conf/mail/$domain/ssl/$domain.pem $SKYNET/ssl/mail/$domain.crt
		ln -s $HOMEDIR/$user/conf/mail/$domain/ssl/$domain.key $SKYNET/ssl/mail/$domain.key
	fi
	...
	# Add domain SSL configuration to exim4
	ln -s $HOMEDIR/$user/conf/mail/$domain/ssl/$domain.pem $SKYNET/ssl/mail/mail.$domain.crt
	ln -s $HOMEDIR/$user/conf/mail/$domain/ssl/$domain.key $SKYNET/ssl/mail/mail.$domain.key
	...
	chmod -R 0644 $SKYNET/ssl/mail/*
	chown -h $user:mail $SKYNET/ssl/mail/*
}
...
del_mail_ssl_config() {
	...
	rm -f $HOMEDIR/$user/conf/mail/$domain/ssl/*
	if [ -n "$mail_cert_match" ]; then
		rm -f $SKYNET/ssl/mail/$domain.crt $SKYNET/ssl/mail/$domain.key
	fi
	rm -f $SKYNET/ssl/mail/mail.$domain.crt $SKYNET/ssl/mail/mail.$domain.key
}
...
is_valid_extension() {
	if [ ! -e "$SKYNET/data/extensions/public_suffix_list.dat" ]; then
		mkdir $SKYNET/data/extensions/
		chmod 750 $SKYNET/data/extensions/
		/usr/bin/wget --tries=3 --timeout=15 --read-timeout=15 --waitretry=3 --no-dns-cache --quiet -O $SKYNET/data/extensions/public_suffix_list.dat https://raw.githubusercontent.com/publicsuffix/list/master/public_suffix_list.dat
	fi
	test_domain=$(idn2 -d "$1")
	extension=$(/bin/echo "${test_domain}" | /usr/bin/rev | /usr/bin/cut -d "." --output-delimiter="." -f 1 | /usr/bin/rev)
	exten=$(grep "^$extension\$" $SKYNET/data/extensions/public_suffix_list.dat)
}
...
is_base_domain_owner() {
	for object in ${1//,/ }; do
		if [ "$object" != "none" ]; then
			...
			web=$(grep -F -H -h "DOMAIN='$basedomain'" $SKYNET/data/users/*/web.conf)

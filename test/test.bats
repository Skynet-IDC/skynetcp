...
function setup() {
    ...
    if [ $BATS_TEST_NUMBER = 1 ]; then
        ...
        echo 'pguser=test5290' >> /tmp/skynet-test-env.sh
        echo 'pgdatabase=test5290_database' >> /tmp/skynet-test-env.sh
        echo 'pgdbuser=test5290_dbuser' >> /tmp/skynet-test-env.sh
    fi
...
function validate_database(){
    ...
    host_str=$(grep "HOST='localhost'" $SKYNET/conf/$type.conf)
...
function check_ip_banned(){
  ...
  run grep "IP='$ip' CHAIN='$chain'" $SKYNET/data/firewall/banlist.conf
  assert_success
...
@test "User: Add new user Failed 1" {
	run v-add-user 'jäap' $user $user@skynetcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}
@test "User: Add new user Failed 2" {
	run v-add-user 'ëaap' $user $user@skynetcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}

@test "User: Add new user Failed 3" {
	run v-add-user 'jaaẞ'  $user $user@skynetcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}

@test "User: Add new user Failed 4" {
	run v-add-user '1234'  $user $user@skynetcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}

@test "User: Add new user Failed 5" {
	run v-add-user '1aap'  $user $user@skynetcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}

@test "User: Add new user Failed 6" {
	run v-add-user 'ib_Buffer'  $user $user@skynetcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: The user name'
}

@test "User: Add new user Failed 7" {
	run v-add-user 'hello.com'  $user $user@skynetcp2.com default "Super Test"
	assert_failure $E_INVALID
	assert_output --partial 'Error: invalid user format'
}
...
@test "User: Add new user Success 1" {
	run v-add-user 'jaap01'  $user $user@skynetcp2.com default "Super Test"
	assert_success
	refute_output
}
...
@test "User: Add new user Success 2" {
	run v-add-user 'buffer'  $user $user@skynetcp2.com default "Super Test"
	assert_success
	refute_output
}
...
@test "User: Change user email" {
    run v-change-user-contact "$user" tester@skynetcp.com
    assert_success
    refute_output
}

@test "User: Change user contact invalid email " {
    run v-change-user-contact "$user" testerskynetcp.com
    assert_failure $E_INVALID
...
@test "Ip: Add new ip on first interface" {
    ...
    # Save initial state
    echo "interface=${interface}" >> /tmp/skynet-test-env.sh
    [ -f "$a2_rpaf" ]     && file_hash1=$(cat $a2_rpaf     |md5sum |cut -d" " -f1) && echo "a2_rpaf_hash='${file_hash1}'"     >> /tmp/skynet-test-env.sh
    [ -f "$a2_remoteip" ] && file_hash2=$(cat $a2_remoteip |md5sum |cut -d" " -f1) && echo "a2_remoteip_hash='${file_hash2}'" >> /tmp/skynet-test-env.sh
    ...
    assert_file_exist $SKYNET/data/ips/$ip
    assert_file_contains $SKYNET/data/ips/$ip "OWNER='$user'"
    assert_file_contains $SKYNET/data/ips/$ip "INTERFACE='$interface'"
...
@test "Ip: [Ubuntu] Netplan file updated" {
   ...
   assert_file_exist /etc/netplan/60-skynet.yaml

   # also check if file contains the newly added ip
   assert_file_contains /etc/netplan/60-skynet.yaml "$ip"
}
...
@test "Ip: Delete ip 198.18.0.12" {
    ...
    assert_file_not_exist $SKYNET/data/ips/$ip
}
...
@test "WEB: Add web domain" {
    ...
    echo -e "<?php\necho 'skynet Test:'.(4*3);" > $HOMEDIR/$user/web/$domain/public_html/php-test.php
    validate_web_domain $user $domain 'skynet Test:12' 'php-test.php'
...
@test "WEB: Generate Self signed certificate" {
    ssl=$(v-generate-ssl-cert "$domain" "info@$domain" US CA "Orange County" skynetcp IT "mail.$domain" | tail -n1 | awk '{print $2}')
    echo $ssl;
...
@test "WEB: Generate Self signed certificate ASCII idn-tést.eu" {
    run v-generate-ssl-cert "xn--idn-tst-fya.eu" "info@xn--idn-tst-fya.eu" US CA "Orange County" skynetcp IT "mail.xn--idn-tst-fya.eu"
    assert_success
}
...
@test "WEB: Generate Self signed certificate ASCII bløst.рф" {
    run v-generate-ssl-cert "xn--blst-hra.xn--p1ai" "info@xn--blst-hra.xn--p1ai" US CA "Orange County" skynetcp IT "mail.xn--blst-hra.xn--p1ai"
    assert_success
}
...
@test "Multiphp: Change backend version - PHP v5.6" {
    ...
    echo -e "<?php\necho 'skynet-multiphptest:'.PHP_MAJOR_VERSION.'.'.PHP_MINOR_VERSION;" > "$HOMEDIR/$user/web/$multi_domain/public_html/php-test.php"
    validate_web_domain $user $multi_domain "skynet-multiphptest:$test_phpver" 'php-test.php'
    rm "$HOMEDIR/$user/web/$multi_domain/public_html/php-test.php"
}
...
@test "DNS: Add domain record MX" {
    ...
    assert_file_contains "$HOMEDIR/$user/conf/dns/${domain}.db" "mx.skynet.com."
...
@test "MAIL: Add account alias" {
	run v-add-mail-account-alias $user $domain test skynetcprocks
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/aliases "skynetcprocks@$domain"
	refute_output
}

@test "MAIL: Add account alias 2" {
	run v-add-mail-account-alias $user $domain test skynetcprocks2
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/aliases "skynetcprocks2@$domain"
	refute_output
}

@test "MAIL: Add account alias 3" {
	run v-add-mail-account-alias $user $domain test skynetcp
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/aliases "skynetcp@$domain"
	refute_output
}

@test "MAIL: Add account 3" {
	run v-add-mail-account $user $domain skynet "$userpass2"
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/limits "skynet@$domain"
	refute_output
}

@test "MAIL: Add account 4" {
	run v-add-mail-account $user $domain skynetrocks3 "$userpass2"
	assert_success
	assert_file_contains /etc/exim4/domains/$domain/limits "skynetrocks3@$domain"
	refute_output
}
...
@test "MAIL: Add account alias Invalid length" {
	run v-add-mail-account-alias $user $domain test 'skynetcp-really-rocks-but-i-want-to-have-feature-xyz-and-i-want-it-now'
	assert_failure $E_INVALID
}
...
@test "MAIL: Add account alias Invalid 2" {
	run v-add-mail-account-alias $user $domain test 'skynet@test'
	assert_failure $E_INVALID
}
...
@test "MAIL: Delete DKIM" {
    ...
    run grep "RECORD='_domainkey'" "${skynet}/data/users/${user}/dns/${domain}.conf"
    assert_failure
    refute_output

    run grep "RECORD='mail._domainkey'" "${skynet}/data/users/${user}/dns/${domain}.conf"
    assert_failure
    refute_output
}
...
@test "MAIL: Delete DKIM but preserve custom dkim records" {
    ...
    run grep "RECORD='k2._domainkey'" "${skynet}/data/users/${user}/dns/${domain}.conf"
    assert_success
...
@test "Allow Users: User can't add user.user2.com " {
    ...
    run v-add-user $user2 $user2 $user@skynetcp.com default "Super Test"
    assert_success
    refute_output
...
@test "PGSQL: Add database" {
  ...
  run v-add-user $pguser $pguser $user@skynetcp.com default "Super Test"
...
@test "Firewall: Add ip to banlist" {
  run v-add-firewall-ban '1.2.3.4' 'skynet'
  assert_success
  refute_output

  check_ip_banned '1.2.3.4' 'skynet'
}

@test "Firewall: Delete ip to banlist" {
  run v-delete-firewall-ban '1.2.3.4' 'skynet'
  assert_success
  refute_output
  check_ip_not_banned '1.2.3.4' 'skynet'
}
...
echo   "1.2.3.4" >> $SKYNET/data/firewall/excludes.conf
  run v-add-firewall-ban '1.2.3.4' 'skynet'
  rm $SKYNET/data/firewall/excludes.conf
  check_ip_not_banned '1.2.3.4' 'skynet'
}
...
@test "Package: Create new Package" {
    cp $SKYNET/data/packages/default.pkg /tmp/package
    run v-add-user-package /tmp/package skynettest
    assert_success
    refute_output
}

@test "Package: Assign user to new Package" {
    run v-change-user-package  $user skynettest
    assert_success
    refute_output
}
...
@test "Package: Update new Package" {
    sed -i "s/BANDWIDTH='unlimited'/BANDWIDTH='100'/g" /tmp/package
    run v-add-user-package /tmp/package skynettest yes
    assert_success
    refute_output
}
...
@test "Package: Update package of user" {
    ...
    assert_success
    refute_output
    run grep "BANDWIDTH='100'" $SKYNET/data/users/$user/user.conf
    assert_success
    assert_output --partial "100"
}

@test "Package: Copy package Not Exists" {
  run v-copy-user-package skynetdoesnotexists skynettest2
  assert_failure $E_NOTEXIST
}

@test "Package: Copy package" {
  run v-copy-user-package skynettest skynettest2
  assert_success
  refute_output
}
...
@test "Package: Delete package" {
    run v-delete-user-package skynettest
    run v-delete-user-package skynettest2
    rm /tmp/package
    assert_success
    refute_output
    run grep "BANDWIDTH='unlimited'" $SKYNET/data/users/$user/user.conf
    assert_success

...
# - 'SHARED_HOST_FOLDER' will be mounted in the (guest lxc) container at '/home/ubuntu/source/' and skynetcp src folder is expected to be there
...
#   cd $SHARED_HOST_FOLDER && git clone https://github.com/skynetcp/skynetcp.git && cd skynetcp && git checkout ..branch..
#

/*
# Nginx reverse proxy config: /etc/nginx/conf.d/lxc-skynet.conf
server {
    listen 80;
...
array_walk($containers, function (&$element) {
	...
	// $hst_args .= ' --with-debs /home/ubuntu/source/skynetcp/src/pkgs/develop/' . $element['os'];
	$hst_args .= " --with-debs /tmp/skynetcp-src/debs";
...
function getskynetVersion($branch) {
	$control_file = "";
	if ($branch === "~localsrc") {
	...
	throw new Exception("Error reading skynet version for branch: [${branch}]", 1);
}
...
function check_lxc_container($container) {
	...
	exec(
		"lxc config device add " .
			escapeshellarg($container["lxc_name"]) .
			" skynetsrc disk path=/home/ubuntu/source source=" .
			SHARED_HOST_FOLDER .
			" 2>/dev/null",
		$devnull,
		$rc,
...
function hst_installer_worker($container) {
	...
	$hver = getskynetVersion(HST_BRANCH);
	echo "Install skynet ${hver} on " . $container["lxc_name"] . PHP_EOL;
	echo "Args: " . $container["hst_args"] . PHP_EOL;

	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "cd \"/home/ubuntu/source/skynetcp\"; install/' .
...
foreach ($containers as $container) {
	...
	# Allow running a reverse proxy in front of skynet
	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "sed -i \'s/session.cookie_secure] = on\$/session.cookie_secure] = off/\' /usr/local/skynet/php/etc/php-fpm.conf"',
	);
	...
	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "sed -i \'/LE_STAGING/d\' /usr/local/skynet/conf/skynet.conf"',
	);
	system(
		"lxc exec " .
			$container["lxc_name"] .
			' -- bash -c "echo \'LE_STAGING=\"yes\"\' >> /usr/local/skynet/conf/skynet.conf"',
	);

	system("lxc exec " . $container["lxc_name"] . ' -- bash -c "service skynet restart"');
}

echo "skynet containers configured" . PHP_EOL;

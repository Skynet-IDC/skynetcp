...
const sessionName = 'skynetSID';
const hostname = execSync('hostname', { silent: true }).toString().trim();
const systemIPs = JSON.parse(
	execSync(`${process.env.skynet}/bin/v-list-sys-ips json`, { silent: true }).toString(),
);
const { config } = JSON.parse(
	execSync(`${process.env.skynet}/bin/v-list-sys-config json`, { silent: true }).toString(),
);
...
wss.on('connection', (ws, req) => {
	...
	const file = readFileSync(`${process.env.skynet}/data/sessions/sess_${sessionID}`);
	if (!file) {

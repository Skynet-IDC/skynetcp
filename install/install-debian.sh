...
if [ "$type" = "ubuntu" ]; then
	check_result 1 "You are running the wrong installer for Ubuntu. Please run hst-install.sh or install-ubuntu.sh instead."
...
if [ -z "$withdebs" ] || [ ! -d "$withdebs" ]; then
	release_branch_ver=$(curl -s https://raw.githubusercontent.com/Skynet-IDC/skynetcp/release/src/deb/skynet/control | grep "Version:" | awk '{print $2}')

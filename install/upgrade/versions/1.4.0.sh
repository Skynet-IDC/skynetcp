...
# skynet Control Panel upgrade script for target version 1.4.0
...
if [ "$API" = "no" ]; then
	if [ -f "$SKYNET/web/api/index.php" ]; then
		echo "[ * ] Disabling API access..."
		$SKYNET/bin/v-change-sys-api remove
	fi
fi

# Back up users existing configuration data to $SKYNET/conf/defaults/skynet.conf
if [ ! -f "$SKYNET/conf/defaults/skynet.conf" ]; then
	...
	cp -f $SKYNET/conf/skynet.conf $SKYNET/conf/defaults/skynet.conf
fi
...
if [ "$WEB_SYSTEM" = "nginx" ]; then
	...
	sed -i "s|TPL='drupal6'|TPL='drupal'|g" $SKYNET/data/users/*/web.conf
	sed -i "s|TPL='drupal7'|TPL='drupal'|g" $SKYNET/data/users/*/web.conf
	sed -i "s|TPL='drupal8'|TPL='drupal'|g" $SKYNET/data/users/*/web.conf
	sed -i "s|TPL='codeigniter2'|TPL='codeigniter'|g" $SKYNET/data/users/*/web.conf
	sed -i "s|TPL='codeigniter3'|TPL='codeigniter'|g" $SKYNET/data/users/*/web.conf
fi

# Remove outdated nginx templates
echo "[ * ] Removing outdated nginx templates..."
rm -rf $SKYNET/data/templates/web/nginx/php-fpm/drupal6.*tpl
rm -rf $SKYNET/data/templates/web/nginx/php-fpm/drupal7.*tpl
rm -rf $SKYNET/data/templates/web/nginx/php-fpm/drupal8.*tpl
rm -rf $SKYNET/data/templates/web/nginx/php-fpm/codeigniter2.*tpl
rm -rf $SKYNET/data/templates/web/nginx/php-fpm/codeigniter3.*tpl

# Clean up old skynet controlled webapps
if [ -d "$SKYNET/web/images/webapps/" ]; then
	echo "[ * ] Clean up old web apps code..."
	rm -rf $SKYNET/web/images/webapps/
	rm -rf $SKYNET/web/src/app/WebApp/Installers/LaravelSetup.php
	rm -rf $SKYNET/web/src/app/WebApp/Installers/OpencartSetup.php
	rm -rf $SKYNET/web/src/app/WebApp/Installers/PrestashopSetup.php
	rm -rf $SKYNET/web/src/app/WebApp/Installers/SymfonySetup.php
	rm -rf $SKYNET/web/src/app/WebApp/Installers/WordpressSetup.php
	rm -rf $SKYNET/web/src/app/WebApp/Installers/Joomla
fi

# Update ClamAV configuration file
if [ -f "/etc/clamav/clamd.conf" ]; then
	cp -f $SKYNET_INSTALL_DIR/clamav/clamd.conf /etc/clamav/
	$SKYNET/bin/v-add-user-notification admin 'ClamAV config has been overwritten' 'Warning: If you have manualy changed /etc/clamav/clamd.conf and any changes you made will be lost an backup has been created in the /root/hst_backups folder with the original config. If you have not changed the config file you can ignore this message'
fi

##### COMMANDS FOR V1.5.X

# Back up default package and install latest version
if [ -d $SKYNET/data/packages/ ]; then
	echo "[ * ] Migrating legacy default package for all users..."
	$SKYNET/bin/v-rename-user-package default custom > /dev/null 2>&1
	echo "[ * ] Replacing default package..."
	cp -f $SKYNET_INSTALL_DIR/packages/default.pkg $SKYNET/data/packages/
fi

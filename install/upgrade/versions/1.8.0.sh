...
# skynet Control Panel upgrade script for target version 1.8.0
...
if ! grep -qw "^[skynet_openssl_sect]$" /etc/ssl/openssl.cnf 2> /dev/null; then
...
if [ "$WEB_SYSTEM" = "nginx" ] || [ "$PROXY_SYSTEM" = "nginx" ]; then
	...
	if ! grep -q "quic_bpf" /etc/nginx/nginx.conf && ! grep -q "spdy_headers_comp" /etc/nginx/nginx.conf; then
		...
		if [ "$nginx_conf_compare" != "same" ]; then
			...
			add_upgrade_message "Manual Action Required [IMPORTANT]\n\nTo enable the \"Enhanced and Optimized TLS\" feature, we must update the NGINX configuration file (/etc/nginx/nginx.conf).\n\nBut for unknown reason or you edited it, may not be fully apply all the changes in this upgrade.\n\nPlease follow the default configuration file to sync it:\n$SKYNET_INSTALL_DIR/nginx/nginx.conf\n\nBacked up configuration file:\n$SKYNET_BACKUP/conf/nginx/nginx.conf\n\nLearn more:\nhttps://github.com/skynetcp/skynetcp/pull/3555\n\n"
			"$BIN"/v-add-user-notification admin "IMPORTANT: Manual Action Required" '<p>To enable the "Enhanced and Optimized TLS" feature, we must update the NGINX configuration file at <code>/etc/nginx/nginx.conf</code>.</p><p>But for unknown reason or you edited it, may not be fully apply all the changes in this upgrade.</p><p>Please follow the default configuration file to sync it:<br><code>'"$SKYNET_INSTALL_DIR"'/nginx/nginx.conf</code></p><p>Backed up configuration file:<br><code>'"$SKYNET_BACKUP"'/conf/nginx/nginx.conf</code></p><p>Visit PR <a href="https://github.com/skynetcp/skynetcp/pull/3555" target="_blank">#3555</a> on GitHub to learn more.</p>'

			sed -i "s/""$(grep -m 1 "IMPORTANT: Manual Action Required" "$SKYNET"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='1'/" "$SKYNET"/data/users/admin/notifications.conf
		...
		add_upgrade_message "About TLS 1.3 0-RTT anti-replay for NGINX\n\nIf you use custom templates, please update them (*.stpl) to apply this protection.\n\nFollow the usage or other default templates:\n/etc/nginx/conf.d/0rtt-anti-replay.conf\n\nLearn more:\nhttps://github.com/skynetcp/skynetcp/pull/3692"
		"$BIN"/v-add-user-notification admin "About TLS 1.3 0-RTT anti-replay for NGINX" '<p>If you use custom templates, please update them (*.stpl) to apply this protection.</p><p>Follow the usage or other default templates:<br><code>/etc/nginx/conf.d/0rtt-anti-replay.conf</code></p><p>Visit PR <a href="https://github.com/skynetcp/skynetcp/pull/3692" target="_blank">#3692</a> on GitHub to learn more.</p>'

		if grep -qw "IMPORTANT: Manual Action Required" "$SKYNET"/data/users/admin/notifications.conf 2> /dev/null; then
			sed -i "s/""$(grep -m 1 "About TLS 1.3 0-RTT anti-replay for NGINX" "$SKYNET"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='2'/" "$SKYNET"/data/users/admin/notifications.conf
		else
			sed -i "s/""$(grep -m 1 "About TLS 1.3 0-RTT anti-replay for NGINX" "$SKYNET"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='1'/" "$SKYNET"/data/users/admin/notifications.conf
		fi
		...
		cp -f "$SKYNET_INSTALL_DIR"/nginx/phpmyadmin.inc "$SKYNET_INSTALL_DIR"/nginx/phppgadmin.inc "$SKYNET_INSTALL_DIR"/nginx/status.conf /etc/nginx/conf.d
...
if [ "$WEB_SYSTEM" = "nginx" ]; then
	while IFS= read -r IP; do
		...
		cp -f "$SKYNET_INSTALL_DIR"/nginx/unassigned.inc "$ip_conf"
		sed -i "s/directIP/$IP/g" "$ip_conf"
		process_http2_directive "$ip_conf"
	done < <(ls "$SKYNET"/data/ips/ 2> /dev/null)
...
if [ "$MAIL_SYSTEM" = "exim4" ]; then
	...
	if version_ge "$exim_version" "4.95"; then
		if ! grep -q 'SRS_SECRET' /etc/exim4/exim4.conf.template; then
			...
			patch /etc/exim4/exim4.conf.template.staging $SKYNET/install/upgrade/patch/3661-exim-srs-support.patch 2>&1
			exim -C /etc/exim4/exim4.conf.template.staging 2>&1
			if [ "$?" -ne 0 ]; then
				add_upgrade_message "Unable to successfully aply the SRS update patch for Exim.\n If you use SMTP relay with the SRS feature use the exim config found in /usr/local/skynet/install/deb/exim/exim4.conf.4.95.template"
				"$BIN"/v-add-user-notification admin "Unable to apply patch to Exim config" 'Unable to successfully apply the SRS update patch for Exim.<br /> If you use SMTP relay with the SRS feature use the exim config found in /usr/local/skynet/install/deb/exim/exim4.conf.4.95.template'
				if grep -qw "IMPORTANT: Manual Action Required" "$SKYNET"/data/users/admin/notifications.conf 2> /dev/null; then
					sed -i "s/""$(grep -m 1 "Unable to apply patch to Exim config" "$SKYNET"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='3'/" "$SKYNET"/data/users/admin/notifications.conf
				else
					sed -i "s/""$(grep -m 1 "Unable to apply patch to Exim config" "$SKYNET"/data/users/admin/notifications.conf | awk '{print $1}')""/NID='2'/" "$SKYNET"/data/users/admin/notifications.conf
				fi

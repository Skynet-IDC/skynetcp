...
# skynet Control Panel upgrade script for target version 1.5.5
...
for version in $($BIN/v-list-sys-php plain); do
	cp -f $SKYNET_INSTALL_DIR/php-fpm/php-fpm.conf /etc/php/$version/fpm/
...
if [ ! -f "$SKYNET/data/packages/system.pkg" ]; then
	echo "[ * ] Install default system package."
	cp -f $SKYNET/install/deb/packages/system.pkg $SKYNET/data/packages/system.pkg
fi

...
# skynet Control Panel upgrade script for target version 1.6.0
...
if [ "$MAIL_SYSTEM" = "exim4" ]; then
	...
	cp $SKYNET_INSTALL_DIR/exim/limit.conf /etc/exim4/limit.conf
	cp $SKYNET_INSTALL_DIR/exim/system.filter /etc/exim4/system.filter
...
if [ -z "$(grep v-update-lets $SKYNET/data/users/admin/cron.conf)" ]; then
...
cp -rf $SKYNET_INSTALL_DIR/api $SKYNET/data/
...
if [ -f "/etc/cron.d/skynet-sftp" ]; then
	rm /etc/cron.d/skynet-sftp
	echo "@reboot root sleep 60 && /usr/local/skynet/bin/v-add-sys-sftp-jail > /dev/null" > /etc/cron.d/skynet-sftp
fi

ips=$(ls /usr/local/skynet/data/ips/ | wc -l)
release=$(lsb_release -s -i)
if [ $release = 'Ubuntu' ]; then
	if [ $ips -gt 1 ]; then
		add_upgrade_message "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. See https://github.com/skynetcp/skynetcp/pull/2612#issuecomment-1135571835 for more info regarding this issue!"
		$SKYNET/bin/v-add-user-notification admin "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. <a href='https://github.com/skynetcp/skynetcp/pull/2612#issuecomment-1135571835'>More info</a>"
	fi
fi

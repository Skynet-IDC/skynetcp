...
# skynet Control Panel upgrade script for target version 1.4.13
...
# Remove old files/folders from previous versions of skynet Control Panel
if [ -d "$SKYNET/web/edit/file/" ]; then
	rm -fr $SKYNET/web/edit/file/
fi
if [ -d "$SKYNET/web/edit/server/theme/" ]; then
	rm -fr $SKYNET/web/edit/server/theme/
fi

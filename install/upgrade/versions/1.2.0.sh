...
# skynet Control Panel upgrade script for target version 1.2.0
...
if [ -d /usr/share/roundcube ]; then
	cp -f $SKYNET_INSTALL_DIR/roundcube/plugins/config_newmail_notifier.inc.php /etc/roundcube/plugins/newmail_notifier/config.inc.php
	cp -f $SKYNET_INSTALL_DIR/roundcube/plugins/config_zipdownload.inc.php /etc/roundcube/plugins/zipdownload/config.inc.php
...
[ ! -f "touch $SKYNET/data/queue/daily.pipe" ] && touch $SKYNET/data/queue/daily.pipe
...
# Add skynet-event.conf, if the server is running apache2
if [ "$WEB_SYSTEM" = "apache2" ]; then
	echo "[ * ] Updating Apache2 configuration..."
	# Cleanup
	rm --force /etc/apache2/mods-available/skynet-event.conf
	rm --force /etc/apache2/mods-enabled/skynet-event.conf
	rm --force /etc/apache2/conf-available/skynet-event.conf
	rm --force /etc/apache2/conf-enabled/skynet-event.conf

	if [ $(a2query -M) = 'event' ] && [ ! -e "/etc/apache2/conf.d/skynet-event.conf" ]; then
...
if [ -z "$FILE_MANAGER_CHECK" ]; then
	if [ ! -e "$SKYNET/web/fm/configuration.php" ]; then
		...
		$SKYNET/bin/v-change-sys-config-value 'FILE_MANAGER' 'true'
	fi
fi
...
for user in $($SKYNET/bin/v-list-sys-users plain); do
...
GZIP_LVL_CHECK=$(cat $SKYNET/conf/skynet.conf | grep BACKUP_GZIP)
...
if [ -e "/var/lib/phpmyadmin" ]; then
	PMA_ALIAS_CHECK=$(cat $SKYNET/conf/skynet.conf | grep DB_PMA_ALIAS)
	if [ -z "$PMA_ALIAS_CHECK" ]; then
		...
		$SKYNET/bin/v-change-sys-db-alias "pma" "$DB_PMA_ALIAS"
	fi
fi

if [ -e "/var/lib/phppgadmin" ]; then
	PGA_ALIAS_CHECK=$(cat $SKYNET/conf/skynet.conf | grep DB_PGA_ALIAS)
	if [ -z "$PGA_ALIAS_CHECK" ]; then
		...
		$SKYNET/bin/v-change-sys-db-alias "pga" "$DB_PGA_ALIAS"
	fi
fi

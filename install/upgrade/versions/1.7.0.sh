...
if [ "$FILE_MANAGER" = "true" ]; then
	...
	$SKYNET/bin/v-delete-sys-filemanager quiet
...
for package in $packages; do
	if [ -z "$(grep -e 'RATE_LIMIT' $SKYNET/data/packages/$package)" ]; then
		echo "RATE_LIMIT='200'" >> $SKYNET/data/packages/$package
	fi
done
...
for file in $SKYNET/data/templates/web/php-fpm/*; do
	echo "[ * ] Update $file legacy /var/run/ to /run/..."
	sed -i 's|/var/run/|/run/|g' $file
done
...
if [ -f /etc/logrotate.d/httpd-prerotate/awstats ]; then
	echo "[ * ] Update Awstats prerotate to skynet update method..."
	# Replace awstatst function
	cp -f $SKYNET_INSTALL_DIR/logrotate/httpd-prerotate/awstats /etc/logrotate.d/httpd-prerotate/
fi

if [ "$PHPMYADMIN_KEY" != "" ]; then
	echo "[ * ] Refresh skynet-sso for PMA..."

...
# skynet Control Panel upgrade script for target version 1.8.11
...
if [ -d "/var/lib/snappymail" ]; then
	chown skynetmail:skynetmail /var/lib/snappymail
	chown skynetmail:skynetmail /etc/snappymail
fi
...
if [ -d "$RC_INSTALL_DIR" ]; then
	chown -R skynetmail:www-data "$RC_INSTALL_DIR"
fi
...
if [ -d "$RC_LOG" ]; then
	chown -R skynetmail:www-data "$RC_LOG"
fi

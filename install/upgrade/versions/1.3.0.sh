...
# skynet Control Panel upgrade script for target version 1.3.0
...
if [ ! -e "/etc/cron.d/skynet-proc" ]; then
...
# Set LOGIN_STYLE variable in skynet.conf
echo "[ * ] Updating configuration file: skynet.conf..."
$BIN/v-change-sys-config-value "LOGIN_STYLE" "default"

# Remove old lanugage files.
if [ -e $SKYNET/web/inc/i18n/en.php ]; then
	echo "[ * ] Performing language library clean-up..."
	rm -fr $SKYNET/web/inc/i18n
fi

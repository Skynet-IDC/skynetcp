...
2. On your skynet server run the following command (`mysql` may be replaced by `postgresql`):
...
Make sure the API is enabled and working properly. skynet’s PhpMyAdmin Single Sign On function connects over the skynet API.
...
```bash
v-add-database-host mysql db.skynetcp.com root mypassword 500
```
...
See `/usr/local/skynet/install/deb/phpmyadmin/pma.sh`

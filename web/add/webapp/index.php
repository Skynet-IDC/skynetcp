...
if (!empty($_GET["app"])) {
	...
	$skynet = new \Skynet\System\SkynetApp();
	$app_installer_class = "\skynet\WebApp\Installers\\" . $app . "\\" . $app . "Setup";
	if (class_exists($app_installer_class)) {
		try {
			$app_installer = new $app_installer_class($v_domain, $SKYNET);
			...
			if ($info["enabled"] == true) {
				$installer = new \skynet\WebApp\AppWizard($app_installer, $v_domain, $SKYNET);
	...
	foreach ($appInstallers as $app) {
		...
		if (
			...
			if ($matches[1] != "Resources") {
				$app_installer_class =
					"\skynet\WebApp\Installers\\" . $matches[1] . "\\" . $matches[1] . "Setup";
				$app_installer = new $app_installer_class($v_domain, $SKYNET);

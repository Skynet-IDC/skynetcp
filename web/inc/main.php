...
define("skynet_DIR_BIN", "/usr/local/skynet/bin/");
define("SKYNET_CMD", "/usr/bin/sudo /usr/local/skynet/bin/");
define("DEFAULT_PHP_VERSION", "php-" . exec('php -r "echo substr(phpversion(),0,3);"'));

// Load skynet Config directly
load_skynet_config();
...
if (
	...
	exec(SKYNET_CMD . "v-log-user-logout " . $v_user . " " . $v_session_id, $output, $return_var);
	destroy_sessions();
...
if (isset($_SESSION["user"])) {
	...
	exec(SKYNET_CMD . "v-list-user " . quoteshellarg($username) . " json", $output, $return_var);
...
if (!defined("NO_AUTH_REQUIRED")) {
	if (empty($_SESSION["LAST_ACTIVITY"]) || empty($_SESSION["INACTIVE_SESSION_TIMEOUT"])) {
		...
		$v_session_id = quoteshellarg($_SESSION["token"]);
		exec(
			SKYNET_CMD . "v-log-user-logout " . $v_user . " " . $v_session_id,
			$output,
			$return_var,
		);
		destroy_sessions();
...
function top_panel($user, $TAB) {
	$command = SKYNET_CMD . "v-list-user " . $user . " 'json'";
...
function is_it_mysql_or_mariadb() {
	exec(SKYNET_CMD . "v-list-sys-services json", $output, $return_var);
	$data = json_decode(implode("", $output), true);
	unset($output);
	$mysqltype = "mysql";
...
function backendtpl_with_webdomains() {
	exec(SKYNET_CMD . "v-list-users json", $output, $return_var);
	...
	foreach ($users as $user => $user_details) {
		exec(
			SKYNET_CMD . "v-list-web-domains " . quoteshellarg($user) . " json",
			$output,
			$return_var,
		);
